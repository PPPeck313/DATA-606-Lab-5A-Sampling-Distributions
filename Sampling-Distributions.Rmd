...
analyzeSamplingProportion <- function(sample, size = NULL) {
  ...
  colnames(sizedSampleTable) <- c("n")
  
  sizedSampleTable <- sizedSampleTable %>%
    mutate(p = n / sum(n)) %>%
    mutate(scientist_work = rowNames) %>%
...
rowNames = c("Benefits", "Doesn't benefit")
...
### Exercise 4
```{r}
analyzeSamplingProportionDistribution <- function(sample, size, reps, binwidth = .02, benefits = FALSE) {
  sizedSamples <- sample %>%
    rep_sample_n(size = size, reps = reps, replace = TRUE) %>%
    count(scientist_work) %>%
    mutate(p_hat = n /sum(n))
  
  typeLabel <- if (benefits) { 
      benefitsLabel
    } else {
      doesntBenefitLabel
    }
  
  sample_props_doesnt_benefit <- sizedSamples %>%
    filter(scientist_work == typeLabel)
  
  print(ggplot(data = sample_props_doesnt_benefit, aes(x = p_hat)) +
  ...
  print(sample_props_doesnt_benefit)
  print(sample_props_doesnt_benefit$p_hat %>%
    describe)
